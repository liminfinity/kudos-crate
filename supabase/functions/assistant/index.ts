...
const PAGE_CONTEXT: Record<string, string> = {
  "/feedback/new": "Пользователь на странице создания отзыва. Помоги составить конструктивный отзыв. Объясни, что текст будет автоматически проверен AI на орфографию и тон.",
  "/kudos/new": "Пользователь на странице Kudos — благодарности коллегам. Помоги выразить признательность. Расскажи про категории: помог разобраться, эмоциональная поддержка, спас дедлайн, поделился экспертизой, менторство, поддержка команды.",
  "/mood": "Пользователь на странице Company Mood Index. CSI = (Positive - Negative) / Total. Зелёный = хорошо (>0.3), жёлтый = нейтрально, красный = проблемы (<-0.1). Помоги интерпретировать данные.",
  "/dashboard": "Пользователь на аналитическом дашборде. Здесь: распределение по сентименту, топ подкатегорий, временная линия, тепловая карта взаимодействий. Объясни метрики.",
  "/kudos/dashboard": "Пользователь на дашборде Kudos. Help Index = количество kudos сотрудника / среднее по компании. Показывает вклад в поддержку команды.",
  ...
  "/incidents": "Пользователь на странице Critical Incidents (только для HR/Admin). Объясни логику отслеживания серьёзных инцидентов.",
...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `Ты — Мира, цифровой ассистент платформы «Опросница» — корпоративной системы обратной связи и аналитики команд.
